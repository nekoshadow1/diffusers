...
from SyncDreamer.ldm.base_utils import read_pickle, concat_images_list
from SyncDreamer.ldm.models.diffusion.sync_dreamer_utils import get_warp_coordinates, create_target_volume
from SyncDreamer.ldm.models.diffusion.sync_dreamer_network import NoisyTargetViewEncoder, SpatialTime3DNet, FrustumTV3DNet
from SyncDreamer.ldm.modules.diffusionmodules.util import make_ddim_timesteps, timestep_embedding
from SyncDreamer.ldm.modules.encoders.modules import FrozenCLIPImageEmbedder
from SyncDreamer.ldm.util import instantiate_from_config
...
class UNetWrapper(nn.Module):
    ...
    def forward(self, x, t, clip_embed, volume_feats, x_concat, down_block_additional_residuals, mid_block_additional_residual, is_train=False):
        """
        ...
        pred = self.diffusion_model(x, t, clip_embed, down_block_additional_residuals, mid_block_additional_residual, source_dict=volume_feats)
        return pred

    def predict_with_unconditional_scale(self, x, t, clip_embed, volume_feats, x_concat, unconditional_scale, down_block_additional_residuals, mid_block_additional_residual):
        x_ = torch.cat([x] * 2, 0)
        t_ = torch.cat([t] * 2, 0)
        ...
        s, s_uc = self.diffusion_model(x_, t_, clip_embed_, down_block_additional_residuals, mid_block_additional_residual, source_dict=v_).chunk(2)
...
class SyncMultiviewDiffusion(pl.LightningModule):
    def __init__(self, unet_config, scheduler_config,
                 ...
                 clip_image_encoder_path="/home/jupyter/SyncDreamer/ckpt/ViT-L-14.pt",
    ...
    def _init_multiview(self):
        K, azs, _, _, poses = read_pickle(f'SyncDreamer/meta_info/camera-{self.view_num}.pkl')
    ...
    def _init_first_stage(self):
        first_stage_config={
            "target": "SyncDreamer.ldm.models.autoencoder.AutoencoderKL",
            "params": {
                "embed_dim": 4,
    ...
    def sample(self, sampler, batch, cfg_scale, batch_view_num, down_block_additional_residuals, mid_block_additional_residual, return_inter_results=False, inter_interval=50, inter_view_interval=2):
        _, clip_embed, input_info = self.prepare(batch)
        x_sample, inter = sampler.sample(input_info, clip_embed, down_block_additional_residuals, mid_block_additional_residual, unconditional_scale=cfg_scale, log_every_t=inter_interval, batch_view_num=batch_view_num)
...
class SyncDDIMSampler:
    ...
    @torch.no_grad()
    def denoise_apply(self, x_target_noisy, input_info, clip_embed, time_steps, index, unconditional_scale, down_block_additional_residuals, mid_block_additional_residual, batch_view_num=1, is_step0=False):
        """
        @param x_target_noisy:   B,N,4,H,W
        @param input_info:
        @param clip_embed:       B,M,768
        ...
        for ni in range(0, N, batch_view_num):
            ...
            if unconditional_scale!=1.0:
                noise = self.model.model.predict_with_unconditional_scale(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, unconditional_scale, down_block_additional_residuals, mid_block_additional_residual)
            else:
                noise = self.model.model(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_, down_block_additional_residuals, mid_block_additional_residual, is_train=False)
    ...
    @torch.no_grad()
    def sample(self, input_info, clip_embed, down_block_additional_residuals, mid_block_additional_residual, unconditional_scale=1.0, log_every_t=50, batch_view_num=1):
        """
        ...
        for i, step in enumerate(iterator):
            ...
            x_target_noisy = self.denoise_apply(x_target_noisy, input_info, clip_embed, time_steps, index, unconditional_scale, down_block_additional_residuals, mid_block_additional_residual, batch_view_num=batch_view_num, is_step0=index==0)
