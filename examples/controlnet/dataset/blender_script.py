...
def normalize_scene(y_offset, z_offset):
    ...
    offset = offset + Vector([0, y_offset, z_offset])
    # with open('print.txt', 'w') as file:
    #     file.write(str(bbox_min[0]) + ',' + str(bbox_min[1]) + ',' + str(bbox_min[2]) + '\n')
    #     file.write(str(bbox_max[0]) + ',' + str(bbox_max[1]) + ',' + str(bbox_max[2]) + '\n')
    # file.close()
    for obj in scene_root_objects():
        obj.matrix_world.translation += offset
    bpy.ops.object.select_all(action="DESELECT")

# ref: https://blenderartists.org/t/setting-a-render-border-for-each-frame-of-an-animation/1160748
def get_2d_bbox(cam):
    UpdateFrames = 1

    def UpdateBorder(camera, object):
    ...
    for obj in bpy.data.objects:
        if obj.type == 'MESH':
            bboxs.append(UpdateBorder(cam, obj))
    return bboxs
...
def save_images(object_file: str) -> None:
    ...
    normalize_scene(args.y_offset, args.z_offset)
    ...
    bbox_dict = {}
    bbox_dict['y_offset'] = args.y_offset
    bbox_dict['z_offset'] = args.z_offset
    bbox_dict['bboxs'] = {}
    for i in range(args.num_images):
        ...
        # Get 2d bbox
        bboxs = get_2d_bbox(camera)
        bbox_dict['bboxs'][i] = bboxs
        
    with open(os.path.join(args.output_dir, object_uid, '{object_uid}.json'.format(object_uid=object_uid)), 'w') as fp:
